...
void init(void)
{
   ...
   handshake_read_bytes[0] = (uint8_t *) &m1_size;
   handshake_read_sizes[0] = sizeof(m1_size);
   handshake_read_bytes[1] = m1;
   handshake_read_sizes[1] = sizeof(m1);
   handshake_read_bytes[2] = (uint8_t *) &m4enc_size;
   handshake_read_sizes[2] = sizeof(m4enc_size);
   handshake_read_bytes[3] = m4enc;
   handshake_read_sizes[3] = sizeof(m4enc);

   handshake_write_bytes[0] = (uint8_t *) &m2_size;
   handshake_write_sizes[0] = sizeof(m2_size);
   handshake_write_bytes[1] = m2;
   handshake_write_sizes[1] = sizeof(m2);
   handshake_write_bytes[2] = (uint8_t *) &m3enc_size;
   handshake_write_sizes[2] = sizeof(m3enc_size);
   handshake_write_bytes[3] = m3enc;
   handshake_write_sizes[3] = sizeof(m3enc);
...
int main(void)
{
   ...
   memcpy(channel.my_ek_pub, host_ek_pub, 32);
   memcpy(channel.my_ek_sec, host_ek_sec, 32);
