...
static void *connection_handler(void *context)
{
    ...
    do
    {
        ...
        if (ret == SALT_SUCCESS)
        {
            printf("\33[2K\rhost: %*.*s", 0, size, &hndsk_buffer[SALT_OVERHEAD_SIZE]);
            printf("Enter message: ");
        }
    } while(ret == SALT_SUCCESS);
...
static void *write_handler(void *context)
{
    ...
    do
    {
        ...
        if (tx_size > 0) {
            printf("\033[A\33[2K\rclient: %*.*s", 0, tx_size, &tx_buffer[SALT_OVERHEAD_SIZE]);
            ret_code = salt_write(channel, (uint8_t*) tx_buffer, tx_size + SALT_OVERHEAD_SIZE);
        }
